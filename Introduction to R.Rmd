...
```{r }
 ...
 emp_status <- factor(c("unemployed","employed", "selfEmployed" ))
> levels(emp_status)
emp_status

```{r }

emp_status <- factor(c("unemployed","employed", "selfEmployed" ), levels = c("selfEmployed", "employed", "unemployed"))
table(emp_status, levels(emp_status))
...
bank_customers <- data.frame(
  customerID= c(0,1,2,3,4),
  hasaccount= c("NO", "yes", "YES", "YES", NA),
  totalbalance = c(NA, 10,000,100, 2,000)
